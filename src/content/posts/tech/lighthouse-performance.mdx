...
그래서 [**squoosh.app**](https://squoosh.app/) 사이트에서 파일 포맷을 jpg, png보다 압축률이 좋은 [**webp**](https://developers.google.com/speed/webp?hl=ko)로 변환하면서 용량 10MB -> 292KB로 줄였다.
...
폰트는 브라우저가 화면을 렌더링할 때, CSSOM과 DOM트리를 합쳐 렌더 트리를 구성하는 단계에서 리소스 요청을 하는데, 컨텐츠가 화면에 그려질 때 폰트가 다운로드되지 않은 상태라면, 폰트가 적용될 텍스트의 렌더링을 차단한다.
...
**font-display** 속성을 **swap**으로 지정하면 **FOUT** 방식으로 동작하여 CLS 점수에 영향이 가지 않는다.

## 공통

### Serve static assets with an efficient cache policy

변할 가능성이 없어서 매 번 서버로부터 불러올 필요가 없는 정적 컨텐츠(주로 이미지와 폰트)를 캐싱하라는 의미이다.
